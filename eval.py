...
def main():
    ...
    predicted_lines_dict = {}
    gold_lines_dict = {}
    #preprocess: make a dictionary which maps a sentence num to the value:
    for line in predicted_lines:
        splitted_line = line.split('\t')
        predicted_lines_dict[splitted_line[0]] = splitted_line[1] + '\t' + splitted_line[2] + '\t' + splitted_line[3]
    for line in gold_lines:
        splitted_line = line.split('\t')
        gold_lines_dict[splitted_line[0]] = splitted_line[1] + '\t' + splitted_line[2] + '\t' + splitted_line[3]
    ''''    
    ...
    for key in gold_lines_dict:
        current_gold_line = gold_lines_dict[key]
        splitted_gold_line = current_gold_line.split('\t')
        if splitted_gold_line[1].lower() == relation:#number of lines in the gold file has the relation we're intrested int
            ...
            if key in predicted_lines_dict:
                ...
                if predicted_lines_dict[key] == current_gold_line:  # predicted correctly
                    num_of_predicted_correctly += 1
                else:  # recall mistake
                    num_of_recall_mistakes += 1
    #check for precision mistakes
    for key in predicted_lines_dict:
        current_pred_line = predicted_lines_dict[key]
        splitted_pred_line = current_pred_line.split('\t')
        if splitted_pred_line[1].lower() == relation and key not in gold_lines_dict:
            num_of_precision_mistakes += 1
    #calculations:
    accuracy = (float(num_of_predicted_correctly) / float(gold_num_of_lines_has_relation))*100
